...
class GeneticAlgorithmBB84:
    def __init__(self, pop_size, max_basis_length, channel_gain, disturbance_prob, max_cascade_pass, elite_size,
        ...
        """We build a list of all chromosomes in this initial population and ranking of their fitness values.
        Although with each population we will reuse these two variables, 
        it's the only instance when they are filled randomly - later on we shall have different methods
        ...
        for i in range(self.pop_size):  # creating initial generation
            ...
            """
            print(i)  # sprawdzam, jak szybko to działa
        ...
        """In order to perform simulations for the whole population in a parallel manner, this part of the algorithm is 
        performed in a separate loop. Population and fitness ranking will be in a shared memory of parallel processes. Each process will
        perform simulations for a specific subset of chromosomes from the populations, based on number of cores
        available for computation, as follows:
        """
            ...
            with multiprocessing.Manager() as manager:
                ...
                for p in range(number_of_processes):
                    proc = multiprocessing.Process(
                        target=simulation_bb84,
                        args=(
                            self.gain, chromosome.genes.get('length'), chromosome.genes.get('rect_basis_prob'),
                            self.disturbance_prob, chromosome.genes.get()
                        )
                    )

        for i in range(self.pop_size):  # evaluating simulation results
            """Since CASCADE cannot function properly without any kind of error estimation, for which we need to 
            publish same subset of raw key, chromosomes with publication_prob_rect = 0 shall have a
            penal fitness value."""

            fit = -100000.0  # this will be fit. val. if the fitness function is not properly set in __init__


            # self.fitness_ranking.update({'index': a[-1][0], 'fitness value': a[-1][1]})
            self.fitness_ranking.append({'index': i, 'fitness value': fit})

        self.fitness_ranking.sort(key=sort_dict_by_fit, reverse=True)
        print('Initial generation created and evaluated.')
    ...
    def ranking_selection(self, crossover_operator):  # creates new generation, overwrites the old one
        ...
        """
        self.fitness_ranking.sort(key=sort_dict_by_fit, reverse=True)  # may be redundant
        new_generation = []
        ...
        while chromosome_counter < (self.elite_size + 2 * self.parents_pair_no):
            ...
            children = crossover_operator(crossover_point=self.cross_point, parent1=parent1, parent2=parent2)
    ...
    def roulette_wheel_selection(self, crossover_operator):
        ...
        for i in range(self.pop_size):
            ...
            param = random.uniform(0, fit_total)
            fit_sum = 0
            index = 0  # musimy coś wymyślić, żeby najlepszy osobnik nie kopulował zazwyczaj sam ze sobą... chyba ten pseudokod jest chuja wart
            while fit_sum < param and index < self.pop_size:
                fit_sum += self.fitness_ranking[index].get('fitness value')
                index += 1
                    ...
                    case 'factored_fit':
                        if chromosome.genes.get('pub_prob_rect') == 0 or chromosome.genes.get('pub_prob_diag') == 0:
                            ...
                            fit = factored_fit(
                                ...
                                quantum_gain=self.gain
                            )
                    case 'negative_time':
                        if chromosome.genes.get('pub_prob_diag') == 0 and chromosome.genes.get('pub_prob_diag') != 0:
                            fit = fitness_negative_time(
                                disturbance_prob=self.disturbance_prob,
                                quantum_gain=self.gain
                            )
                    case 'fitness_inv':
